...
class SplitImage:
    ...
    def __init__(self, image_path) -> None:
        ...
        self.pixmap = self.create_pixmap_for_gui()
        self.name = pathlib.Path(self.path).stem
        self.flags = self.read_flags_from_filename()
        self.delay_duration = self.read_delay_from_filename()
        self.pause_duration = self.read_pause_from_filename()
    ...
    def create_pixmap_for_gui(self):
        image = QImage(self.ndarray, self.ndarray.shape[1], self.ndarray.shape[0], QImage.Format_RGB888)
